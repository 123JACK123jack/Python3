...
def get_index(offset):
    base_url = "https://www.guokr.com/apis/minisite/article.json?retrieve_type=by_channel&channel_key=frontier&"
    data1 = {
...
def save_database(title,way):
    ...
    a=str(title)
    b=str(way)
    cur.execute("insert into app2_article4(title,way) values(%s,%s)", (a,b))
    conn.commit()
    cur.close()
    return True
