...
def get_index(offset):
    base_url = "https://www.guokr.com/apis/minisite/article.json?retrieve_type=by_subject&subject_key=internet&"
    data1 = {
...
def parse_page2(resp):
    try:
        ...
        pass
    """
def save_database(title):
    cur = conn.cursor()
    a=str(title)
   # b=str(way)
    cur.execute("insert into app2_article6(title) values(%s)", (a))
    conn.commit()
    cur.close()
    return True

if __name__ == '__main__':
    offsets: List[int] = ([0] + [i * 20 + 18 for i in range(20)])
