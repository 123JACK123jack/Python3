...
def parse_page2(resp):
    try:
        ...
        pass
def save_database(title,author):
    cur = conn.cursor()
    a=str(title)
    b=str(author)
    cur.execute("insert into app2_Article2(title,author) values(%s,%s)", (a,b))
    conn.commit()
    cur.close()
    return True

if __name__ == '__main__':
    offsets: List[int] = ([0] + [i * 20 + 18 for i in range(5)])
    for offset in offsets:
        ...
        for url in all_url:
           ...
           # data=parse_page(resp)
            ...
            author = parse_page2(resp)
            print(title,author)
            save_database(title, author)
